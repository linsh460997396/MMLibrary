...
namespace MetalMaxSystem
{
    public partial class MMCore
    {
        ...
        private const string c_tablePrefix = "libBC0D3AAD_gv_";
        #endregion

        #region 状态变量
        private static bool _xuLiGuanLi = false;
        private static bool _shuangJiGuanLi = false;
        private static bool _tiaoShiXuLiZhi = false;
        private static bool _tiaoShiShuangJiZhi = false;
        #endregion
        ...
        {
            return ThreadStringBuilder.Concat(c_tablePrefix, "IntGroup_XuLi", player);
        }
        ...
        public static void UpdateXuLi(int player)
        {
            ...
            for (int i = 1; i <= num; i++)
            {
                ...
                if (_tiaoShiXuLiZhi && KXL != 0)
                {
                    System.Diagnostics.Debug.WriteLine($"P{player}蓄力值[{key}] => {KXL}");
                }
            }
        }
        /// <summary>
        ...
        {
            return ThreadStringBuilder.Concat(c_tablePrefix, "IntGroup_DoubleClicked", player);
        }
        /// <summary>
        ...
        public static void StartTiaoShiXuLiZhi(bool enable)
        {
            _tiaoShiXuLiZhi = enable;
        }
        public static void StartTiaoShiShuangJiZhi(bool enable)
        {
            _tiaoShiShuangJiZhi = enable;
        }
        /// <summary>
        /// 双击管理主循环 - 每帧调用
        ...
        public static void UpdateShuangJi(int player)
        {
            ...
            for (int i = 1; i <= num; i++)
            {
                ...
                if (_tiaoShiShuangJiZhi && KSJ != -1.0f)
                {
                    System.Diagnostics.Debug.WriteLine($"P{player}双击值[{key}] => {KSJ}");
                }
            }
        }
        /// <summary>
        ...
        #region 多玩家遍历版本（可选）
        /// <summary>
        /// 蓄力管理主循环 - 遍历指定玩家范围内的所有玩家
        /// </summary>
        /// <param name="startPlayer">起始玩家ID</param>
        /// <param name="endPlayer">结束玩家ID</param>
        public static void UpdateXuLiForAllPlayers(int startPlayer = 0, int endPlayer = 10)
        {
            for (int player = startPlayer; player <= endPlayer; player++)
            {
                UpdateXuLi(player);
            }
        }
        /// <summary>
        /// 双击管理主循环 - 遍历指定玩家范围内的所有玩家
        /// </summary>
        /// <param name="startPlayer">起始玩家ID</param>
        /// <param name="endPlayer">结束玩家ID</param>
        public static void UpdateShuangJiForAllPlayers(int startPlayer = 0, int endPlayer = 10)
        {
            for (int player = startPlayer; player <= endPlayer; player++)
            {
                UpdateShuangJi(player);
            }
        }
        #endregion
    }

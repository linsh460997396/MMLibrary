namespace MMWorld.Test
{
    ...
    /// MM测试专用怪物生成器
    /// </summary>
    public class MMTestMonsterGenerator : MonsterGenerator
    {
        /// <summary>
        /// [构造函数]创建MM测试专用怪物生成器
        /// </summary>
        ...
        public MMTestMonsterGenerator(string name_, Stage stage_, int activeTime_, int destroyTime_, int generateDelay_) : base(name_, stage_, activeTime_, destroyTime_, generateDelay_) { }
        ...
        public override void Update()
        {
            ...
            if (time > nextGenerateTime)
            {
                ...
                new MMTestMonster(stage).Init(pos.x, pos.y);
            }
        }
    }
}
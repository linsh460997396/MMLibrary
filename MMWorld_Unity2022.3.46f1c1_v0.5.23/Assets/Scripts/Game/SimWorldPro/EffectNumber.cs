...
public class EffectNumber
{
    ...
    public EffectNumber(Stage lv_stage, float lv_pixelX, float lv_pixelY, float lv_scale, double lv_value, bool lv_criticalHit)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            GO.Pop(ref o, 0, "FG2");
            o.r.sprite = scene.sprites_font_outline[sb[i] - 32];//char可直接与数字加减，如sb[i]='2'，结果是(int)'2' -32=50=32=18，对应精灵数组索引[18]，即2的精灵图片
    ...
    public virtual void Draw(float cx, float cy)
    {
        ...
        {//数字特效的逻辑坐标未达到参数值
            for (int i = 0; i < size; ++i)
            {
                ...
                gos[i].t.position = new Vector3((pixelX + i * 8 * scale) / Scene.gridSize, pixelY / Scene.gridSize, 0);
            }
        }
    }

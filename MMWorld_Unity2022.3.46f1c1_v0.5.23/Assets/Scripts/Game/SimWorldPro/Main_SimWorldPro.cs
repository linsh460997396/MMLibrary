...
/// 主入口
/// </summary>
public class Main_SimWorldPro : MonoBehaviour
{
    //[NonSerialized] public static List<Sprite>[] mapSprites; //由于使用了材质纹理UV划区，这个精灵数组可不再使用
    ...
    void Awake()
    {
        ...
        //Test_AIBehavior.Main(); //unsafe模式下测试亿次AI行为
    ...
    void Update()
    {
        ...
        {//幸存者场景组件未启用且单元团块空间停止生成时
            sceneEnabled = true;
            this.gameObject.GetComponent<Scene>().enabled = true;//启用幸存者场景组件
        }
    }
    ...
    void LoadAssets()
    {
        ...
        #region 
        //注意打包前是Assets文件夹下的素材路径，打包后是MMWorld_Data文件夹下的素材路径（这种方式素材打包后可直观修改和替换）
        ...
        //官方Resources方法，Resources文件夹内的素材会自动打包，不用担心打包后路径问题（这种方式素材打包后没法直观修改和替换）

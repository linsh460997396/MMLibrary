...
{
    /// <summary>
    /// 【MM_函数库】常规触发器，创建实例后请给函数注册事件（语法：Trigger.Awake/Start/Update/End/Destroy +=/-= 任意符合事件参数格式的函数的名称如MyFunc，其声明为void MyFun(object sender, EventArgs e)，sender传递本类实例（其他类型也可），e传递额外事件参数类的信息），Run方法将自动创建独立触发器线程并启动常规触发器（主体事件发布动作），启动前可用Duetime、Period属性方法设定Update阶段每次循环的前摇和间隔，启动后按序执行Awake/Start/Update/End/Destroy被这5种事件注册过的委托函数，其中事件Update阶段是一个计时器循环，直到用户手动调用TimerStop属性方法，该属性为false时会让计时器到期退出Update循环，而计时器所在父线程（即触发器线程）将运行End和Destory事件
    /// </summary>
    public class Trigger
    {
        ...
        /// </summary>
        /// <summary>
        /// 【MM_函数库】自动复位事件，提供该属性方便随时读取，属性动作AutoResetEvent_Trigger.Set()可让触发器线程终止（效果等同Trigger.TimerState = true）
        /// </summary>
        public AutoResetEvent AutoResetEvent_Trigger { get; private set; }

        /// <summary>
        /// 【MM_函数库】常规触发器Update事件运行次数，该属性可随时读取或清零
        /// </summary>
        public int InvokeCount { get; set; }

        /// <summary>
        /// 【MM_函数库】常规触发器Update事件运行次数上限，该属性让计时器到期退出循环，计时器所在父线程将运行End和Destory事件
        /// </summary>
        public int InvokeCountMax { get; set; }

        /// <summary>
        /// 【MM_函数库】常规触发器Update事件前摇，未设置直接启动Run则默认为0
        /// </summary>
        public int Duetime { get; set; }

        /// <summary>
        /// 【MM_函数库】常规触发器的运行间隔属性，未设置直接启动Run则默认为1s
        /// </summary>
        public int Period { get; set; }

        /// <summary>
        /// 【MM_函数库】常规触发器的状态属性，手动设置为false则计时器工作时将收到信号退出循环（不执行Update事件），计时器所在父线程将运行End和Destory事件
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】常规触发器主体事件发布动作所在线程的实例，提供该属性方便随时读取，但不允许不安全赋值
        /// </summary>
        public Thread Thread { get; private set; }

        /// <summary>
        /// 【MM_函数库】常规触发器执行Update事件的计时器实例，提供该属性方便随时读取，但不允许不安全赋值
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】将常规触发器的唤醒事件注册到函数，语法：Trigger.Awake +=/-= 实例或静态函数（特征格式：void 函数名(object sender, EventArgs e)）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】将常规触发器的开始事件注册到函数，语法：Trigger.Start +=/-= 实例或静态函数（特征格式：void 函数名(object sender, EventArgs e)）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】将常规触发器的开始事件注册到函数，语法：Trigger.Update +=/-= 实例或静态函数（特征格式：void 函数名(object sender, EventArgs e)）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】将常规触发器的开始事件注册到函数，语法：Trigger.End +=/-= 实例或静态函数（特征格式：void 函数名(object sender, EventArgs e)）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】将常规触发器的开始事件注册到函数，语法：Trigger.Destroy +=/-= 实例或静态函数（特征格式：void 函数名(object sender, EventArgs e)）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】常规触发器主体事件发布动作（重复执行则什么也不做），在当前线程创建常规触发器并执行事件委托（提前定义事件委托变量Trigger.Awake/Start/Update/End/Destroy += 要委托执行的函数，即完成事件注册到函数），可预先自定义计时器Updata阶段的执行间隔（否则默认以Duetime=0、Period=1000运行计时器）。注：若直接调用本函数则在计时器Updata阶段会暂停当前线程，不想暂停请额外开线程手动加载Action运行或使用Trigger.Run自带线程启动（推荐）
        /// </summary>
        private void Action()
        {
        ...
        /// <summary>
        /// 【MM_函数库】自动创建线程启动常规触发器（模拟触发器运行），重复启动时什么也不做，未设置Update属性则默认以Duetime=0、Period=1000运行计时器循环
        /// </summary>

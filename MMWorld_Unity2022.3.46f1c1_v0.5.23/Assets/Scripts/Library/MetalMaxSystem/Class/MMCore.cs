...
namespace MetalMaxSystem
{
    ...
    public static class MMCore
    {
        ...
        /// <summary>
        /// 【MM_函数库】临时跨线程字典<string,Vector> （函数或动作集结束时应手动排泄）
        /// </summary>
        ...
        //在本库中，前缀_开头字段表示其拥有属性方法

        private static bool isDataTableTypeSet = false;
        private static bool _dataTableType = false;
        /// <summary>
        /// 【MM_函数库】数据表类型，为true时全局切换为哈希表（跨线程安全），其他情况默认采用字典（勿跨线程），可通过切换DataTableType来对比读写效率（只允许被改变一次，请在入口或使用数据表前进行设置）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】存储区容错处理当表键值存在时执行线程默认等待的间隔。常用于多线程触发器频繁写值，如大量注册注销动作使存储区数据重排序的，因表正在使用需排队等待完成才给执行下一个。执行原理：将调用该函数的当前线程反复挂起period毫秒，直到动作要写入的存储区闲置
        /// </summary>
        ...
        /// <summary>
        ///  【MM_函数库】以实数返回二维坐标（pixelX,pixelY）与（a,b）形成的角度（单位：度）
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="a"></param>
        ...
        /// <summary>
        /// 【MM_函数库】以实数返回三维坐标（pixelX,pixelY,z）与（a,b,c）形成的角度（单位：度）
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        {
            //double X1 = point1.pixelX, Y1 = point1.pixelY, X2 = point2.pixelY, Y2 = point2.pixelY;
        ...
        /// <summary>
        /// 【MM_函数库】以实数返回二维坐标（pixelX,pixelY）与（a,b）形成的距离（单位：m）
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="a"></param>
        ...
        /// <summary>
        /// 【MM_函数库】以实数返回三维坐标（pixelX,pixelY,z）与（a,b,c）形成的距离（单位：度）
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        /// <summary>
        /// 【MM_函数库】写文本每行（默认UTF-8），文件若不存在则自动新建，StreamWriter默认缓冲区大小为8192个字节（8KB），满时自动写入文件，本函数使用using代码块，StreamWriter对象被关闭时，缓冲区中的数据也会被写入文件。
        /// </summary>
        /// <param name="path"></param>
        ...
        /// <summary>
        /// 【MM_函数库】写文本每行，文件若不存在则自动新建，StreamWriter默认缓冲区大小为8192个字节（8KB），满时自动写入文件，本函数使用using代码块，StreamWriter对象被关闭时，缓冲区中的数据也会被写入文件。
        /// </summary>
        /// <param name="path"></param>
        /// <param name="value"></param>
        /// <param name="torf"></param>
        ...
        /// <summary>
        /// 【MM_函数库】写文本（默认UTF-8），文件若不存在则自动新建，StreamWriter默认缓冲区大小为8192个字节（8KB），满时自动写入文件，本函数使用using代码块，StreamWriter对象被关闭时，缓冲区中的数据也会被写入文件。
        /// </summary>
        /// <param name="path"></param>
        /// <param name="value"></param>
        /// <param name="torf">false是覆盖，true是追加文本</param>
        ...
        /// <summary>
        /// 【MM_函数库】调用File.WriteAllText将文本内存写入文件（若路径不存在会尝试建立）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】调用File.WriteAllBytes将文本内存写入文件（若路径不存在会尝试建立）
        /// </summary>
        /// <param name="fileSavePath"></param>
        /// <param name="content"></param>
        ...
        /// <summary>
        /// 【MM_函数库】从哈希表中移除Key。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        ...
        /// <summary>
        /// 【MM_函数库】从哈希表中移除Key[]，模拟1维数组。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        /// <param name="lp_1"></param>
        ...
        /// <summary>
        /// 【MM_函数库】从哈希表中移除Key[,]，模拟2维数组。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        /// <param name="lp_1"></param>
        ...
        /// <summary>
        /// 【MM_函数库】从哈希表中移除Key[,,]，模拟3维数组。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        /// <param name="lp_1"></param>
        ...
        /// <summary>
        /// 【MM_函数库】从哈希表中移除Key[,,,]，模拟4维数组。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        /// <param name="lp_1"></param>
        ...
        /// <summary>
        /// 【MM_函数库】移除哈希表键值对。注：移除并不效率，如果要重复使用该键可赋空值）
        /// </summary>
        /// <param name="place"></param>
        /// <param name="key"></param>
        ...
        /// <summary>
        /// 【MM_函数库】存储区容错处理函数，当哈希表键值存在时执行线程等待。常用于多线程触发器频繁写值，如大量注册注销动作使存储区数据重排序的，因哈希表正在使用需排队等待完成才给执行下一个。执行原理：将调用该函数的当前线程反复挂起dataTableThreadWaitPeriod毫秒，直到动作要写入的存储区闲置
        /// </summary>
        /// <param name="key"></param>
        ...
        /// <summary>
        /// 【MM_函数库】互动O_注册Object标签句柄并返回，若对象已被注册则返回其句柄，本函数不会进行重复注册
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】互动O_返回Object已注册标签句柄，对象未注册则返回0
        /// </summary>
        /// <param name="lp_object"></param>
        ...
        /// <summary>
        /// 【MM_函数库】互动O_注册Object(高级)。在指定Key存入Object，固有状态、固有自定义值是Object独一无二的标志（本函数重复注册会刷新），之后可用互动O_"返回Object注册总数"、"返回Object序号"、"返回序号对应Object"、"返回序号对应Object标签"、"返回Object自定义值"。Object组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将Object组转为Key。固有状态相当于单位组单位活体，如需另外设置多个标记可使用"互动O_设定Object状态/自定义值"
        /// </summary>
        /// <param name="lp_object"></param>
        ...
        /// <summary>
        /// 【MM_函数库】互动O_注册Object。在指定Key存入Object，固有状态、固有自定义值是Object独一无二的标志（本函数重复注册不会刷新），之后可用互动O_"返回Object注册总数"、"返回Object序号"、"返回序号对应Object"、"返回Object自定义值"。Object组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将Object组转为Key。首次注册时固有状态自动为true（相当于单位组单位活体），之后只能通过"互动O_注册Object（高级）"改写，如需另外设置多个标记可使用"互动O_设定Object状态/自定义值"
        /// </summary>
        /// <param name="lp_object"></param>
        /// <param name="lp_key">存储键区，默认值"_Object"</param>
        ...
        /// <summary>
        /// 【MM_函数库】互动S_注册String(高级)。在指定Key存入String，固有状态、固有自定义值是String独一无二的标志（本函数重复注册会刷新），之后可用互动S_"返回String注册总数"、"返回String序号"、"返回序号对应String"、"返回序号对应String标签"、"返回String自定义值"。String组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将String组转为Key。首次注册时固有状态为true（相当于单位组单位活体），如需另外设置多个标记可使用"互动S_设定String状态/自定义值"
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】互动S_注册String。在指定Key存入String，固有状态、固有自定义值是String独一无二的标志（本函数重复注册不会刷新），之后可用互动S_"返回String注册总数"、"返回String序号"、"返回序号对应String"、"返回String自定义值"。String组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将String组转为Key。首次注册时固有状态为true（相当于单位组单位活体），之后只能通过"互动S_注册String（高级）"改写，如需另外设置多个标记可使用"互动S_设定String状态/自定义值"
        /// </summary>
        /// <param name="lp_string"></param>
        /// <param name="lp_key">存储键区，默认值"_String"</param>
        ...
        public static void HD_ForEachStringFromGroup(string lp_gs, int lp_start, int lp_increment, SubActionEventFuncref lp_funcref)
        {
            ...
            for (; (lv_ai >= 0 && lv_va <= lv_ae) || (lv_ai < 0 && lv_va >= lv_ae); lv_va += lv_ai)
            {
                lv_timer = DataTableStringLoad0(false, "HD_StringGFor" + lv_vs + lv_va.ToString());
        ...
        /// <summary>
        /// 【MM_函数库】互动I_注册Int(高级)。在指定Key存入Int，固有状态、固有自定义值是Int独一无二的标志（本函数重复注册会刷新），之后可用互动I_"返回Int注册总数"、"返回Int序号"、"返回序号对应Int"、"返回序号对应Int标签"、"返回Int自定义值"。Int组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将Int组转为Key。首次注册时固有状态为true（相当于单位组单位活体），如需另外设置多个标记可使用"互动I_设定Int状态/自定义值"
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】互动I_注册Int。在指定Key存入Int，固有状态、固有自定义值是Int独一无二的标志（本函数重复注册不会刷新），之后可用互动I_"返回Int注册总数"、"返回Int序号"、"返回序号对应Int"、"返回Int自定义值"。Int组使用时，Key被强制为变量ID，可用"获取变量的内部名称"将Int组转为Key。首次注册时固有状态为true（相当于单位组单位活体），之后只能通过"互动I_注册Int（高级）"改写，如需另外设置多个标记可使用"互动I_设定Int状态/自定义值"
        /// </summary>
        /// <param name="lp_integer"></param>
        /// <param name="lp_key">存储键区，默认值"_Int"</param>

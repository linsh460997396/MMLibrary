                        ...
                        { // don't render empty blocks.不渲染空块
                          //获取体素数据（体素块的种类）对应的体素（类型）
                            ...
                            { // if not cube.如果不是立方体
                                ...
                                { // if any adjacent cellId isn't opaque, we render the mesh.如果任何相邻体素不是不透明的，自定义渲染网格
        ...
        { // returns true if a face should be spawned
            ...
            else
            {
                ...
                {//执行此检查的体素块完全透明
                    ...
                    {//如果邻块是完全透明
                        // don't draw a transparent block next to another transparent block.禁止在一个完全透明体素块旁边绘制另一个透明块
                        //Debug.Log("2");
                        return false;
                    }
                    else

...
namespace CellSpace
{
    ...
    public class CPEngine : MonoBehaviour
    {
        ...
        /// <summary>
        /// 纹理ID字符列表，0为大地图（第2材质），1~238为小地图（第3材质）
        /// </summary>
        [NonSerialized] public static List<string>[] mapContents = new List<string>[240];
        /// <summary>
        /// 纹理ID列表，0为大地图（第2材质），1~239为小地图（第3材质）
        /// </summary>
        [NonSerialized] public static List<ushort>[] mapIDs = new List<ushort>[240];
        /// <summary>
        /// 存储场景图片宽度信息的数组，供地图创建函数使用
        /// </summary>
        [NonSerialized] public static List<ushort> mapWidths = new List<ushort>();
        ///// <summary>
        ///// 存储碰撞类型为Cube的纹理ID数组，用于开局批创建地块预制体
        ///// </summary>
        //[NonSerialized] public static List<ushort> cubeColliderIDs = new List<ushort>();
        ...
        /// </summary>
        /// <param name="cellID"></param>
        /// <param name="textureRow"></param>
        /// <param name="textureCol"></param>
        /// <param name="subMeshIndex">网格渲染器的材质索引</param>
        void CreateTexPrefabBatch(ushort cellID, ushort subMeshIndex, ushort textureRow, ushort textureCol)
        {
            ushort index = cellID;
            ushort X; // 当前X坐标
            ushort Y = 0; // 当前Y坐标
        ...
        void LoadTXT()
        {
            ...
            for (int i = 0; i <= 238; i++)
            {
                ...
                for (int j = 0; j < fields.Length; j++)
                {
                    ...
                    mapIDs[i + 1].Add(ushort.Parse(fields[j])); //首次添加用Add而不是赋值动作
                }
            }
            //小地图239场景图片宽度信息文本
        ...
        void BlocksRefresh()
        {
            ...
            ushort num1 = (ushort)(num0 + 152);
            ushort num2 = (ushort)(num1 + 1360);
            PrefabOPs = new OP[Blocks.Length]; //GUI界面lBlocks填了1540数组容量，创建同样Blocks.Length个OP对象，它们共享对象池
            //对象池预填充↓
            ...
            CreateTexPrefabBatch(num0, 1, 19, 8);
            for (ushort i = num0; i < num1; i++)
            {
                Blocks[i] = PrefabOPs[i].gameObject;//填充内部调用的Blocks数组（1540个元素，前面11个由GUI读取）
                OP.pool.Push(PrefabOPs[i]);//将OP对象退回栈，待使用时取出
            }
            //额外添加1360个小地图纹理：cell_163~1522（第三材质）
            //↓小地图170行8列会自动转换出1360个预制体，OP对象会绑定这些转化的预制体
            CreateTexPrefabBatch(num1, 2, 170, 8);
            ...
            // 初始化数组中的每个List元素（用于存放重装机兵全场景纹理ID）
        ...
        //Awake方法在脚本实例被加载且处于激活状态时被调用，通常发生在游戏对象被创建或脚本被添加到游戏对象上
        public void Awake()
        {
            //本函数负责引擎初始化
            ...
            {
                HeightRange = 0;
                Debug.LogWarning("CellSpace: CellChunk height range can't be a negative number! Setting chunk height range to 0." +
            ...
            if (MaxChunkDataRequests < 0)
            {
                ...
                Debug.LogWarning("CellSpace: Max chunk data requests can't be a negative number! Setting max chunk data requests to 0." +
        ...
        public static Vector2 GetTextureOffset(ushort cellID, Facing facing)
        {
            ...
            { // if this cell isn't using custom side textures, return the Up texture.如果这个单元没有使用自定义单面纹理，直接返回单元朝上的那面纹理点（6个面共用）

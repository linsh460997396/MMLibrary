...
public class EffectExplosion
{
    ...
    public EffectExplosion(Stage lv_stage, float lv_pixelX, float lv_pixelY, float lv_scale)
    {
        ...
        go.t.SetPositionAndRotation(new Vector3(lv_pixelX / Scene.gridSize, lv_pixelY / Scene.gridSize, 0), Quaternion.identity);
        var s = displayBaseScale * lv_scale; //计算最终缩放量
        go.t.localScale = new Vector3(s, s, s);  //进行缩放
        //记录逻辑坐标
        pixelX = lv_pixelX;
        pixelY = lv_pixelY;
    }
    ...
    {
#if UNITY_EDITOR
        if (go.g != null)           //Unity点击停止按钮后这些变量似乎有可能提前变成null
#endif
        {
            //退回对象池
            GO.Push(ref go);

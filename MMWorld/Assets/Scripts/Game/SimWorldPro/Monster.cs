...
public class Monster : SpaceItem
{
    ...
    protected Monster(Stage stage_)
    {
        ...
        GO.Pop(ref mgo, 3);
        //设置小地图对象的材质和缩放
        mgo.r.material = scene.minimapMaterial;
        mgo.t.localScale = new Vector3(1, 1, 1);
    ...
    public void Init(Sprite[] sprites_, float lv_pixelX, float lv_pixelY)
    {
        ...
        mgo.r.sprite = sprites[0];//动画帧重置为第一个
    ...
    public virtual void Draw(float cx, float cy)
    {
        ...
        {//在屏幕内
            ...
            //同步&坐标系转换
            go.t.position = new Vector3(pixelX / Scene.gridSize, pixelY / Scene.gridSize, 0);
            //同步尺寸缩放(根据半径推送算)
        ...
        else
        {
            ...
            mgo.t.position = new Vector3(pixelX / Scene.gridSize, pixelY / Scene.gridSize, 0);//[渲染层]设置小地图对象位置
        }
    }

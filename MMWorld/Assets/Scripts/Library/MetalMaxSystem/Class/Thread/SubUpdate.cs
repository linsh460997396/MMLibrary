...
namespace MetalMaxSystem
{
    ...
    public static class SubUpdate
    {
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器Update阶段，用来实现周期循环的计时器
        /// </summary>
        public static Timer Timer { get; private set; }//不提供外部赋值

        /// <summary>
        /// 【MM_函数库】副循环触发器自动复位事件对象（用来向副循环触发器线程发送信号），属性动作AutoResetEvent.Set()可让触发器线程终止（效果等同SubUpdate.TimerStop = true）
        /// </summary>
        public static AutoResetEvent AutoResetEvent { get; private set; }//不提供外部赋值

        /// <summary>
        /// 【MM_函数库】副循环触发器Update事件运行次数
        /// </summary>
        public static int InvokeCount { get; set; }

        /// <summary>
        /// 【MM_函数库】副循环触发器状态，手动设置为false则计时器工作时将收到信号退出循环（不执行Update事件），计时器所在父线程将运行End和Destory事件
        /// </summary>
        public static bool TimerStop { get; set; }

        /// <summary>
        /// 【MM_函数库】副循环触发器Update阶段前摇时间，设置后每次循环前都会等待
        /// </summary>
        public static int Duetime { get; set; }
        /// <summary>
        /// 【MM_函数库】副循环触发器Update阶段间隔运行时间
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器状态监控类（用来读写InvokeCount、TimerStop属性），计时器实例创建时本类方法CheckStatus以参数填入被反复执行，副循环触发器Update事件被执行时创建计时器的父线程（SubUpdate.Thread）将暂停，直到该方法确认到TimerStop为真，退出计时器循环，并通知计时器所在父线程恢复运行（将执行End和Destory事件）
        /// </summary>
        static SubUpdate()
        {
            InvokeCount = 0;
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器的计时器实例创建时以参数填入、被反复执行的函数，Update事件被执行时创建计时器的父线程将暂停，直到本函数确认到TimerStop为真，退出计时器循环，并通知计时器所在父线程恢复运行（将执行End和Destory事件）。一般不需要用户操作，TimerStop为true时手动调用会额外增加Update次数
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】开启副循环触发器（默认0.05现实时间秒，如需修改请在开启前用属性方法SubUpdate.Period、SubUpdate.Duetime来调整计时器Update阶段的间隔、前摇，若已经开启想要修改，可使用SubUpdate.Timer.Change）
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器方法，若Update阶段属性未定义则默认每轮前摇0ms、间隔50ms
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器每轮更新运行，主动调用时跟Unity引擎一样只运行一次
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器主体事件发布动作（重复执行则什么也不做），若Update阶段属性未定义则默认每轮前摇0ms、间隔50ms
        /// </summary>
        ...
        /// <summary>
        /// 【MM_函数库】副循环触发器主体事件发布动作（重复执行则什么也不做），可自定义Update阶段属性Duetime（前摇）、Period（间隔）
        /// </summary>
